(function($) {
	...
	var globalFunctions = {
		...
		aux = {
            ...
            $el.addClass('slide-content-empty');  
            ...
            $paginationContainer.css('display', 'none');
				...
				$slides.each(function(i) {
					var $this = $(this),
						href  = $this.find('a.slide-link').attr('href')   //KHAMYL edit 'a' -> 'a.slide-link'
						mode  = $this.find('a.slide-link').attr('class'), //KHAMYL edit 'a' -> 'a.slide-link'
						title = $this.find('a.slide-link').attr('title'); //KHAMYL edit 'a' -> 'a.slide-link'
				...
				//$slides.find('a').remove(); //KHAMYL - edit
				$slides.find('a.slide-link').remove();
			...
			init : function( options ) {
				...
				if( this.length ) {
					...
					return this.each(function() {
						...
						$(window).load(function() {
							if( settings.timeout > 0 && mouseOver !== true && $el.is(":visible"))   //khamyl addde isVisible test
						...
						function swithSlide( $button, $curSlide, $curContent ) {						   
							...
							animTime = setTimeout(function() {
								...
								//KHAMYL - edit: hide contentContainer when it is empty
								if($curContent[0].children.length < 1) {
                  $contentContainer.css('display', 'none');
                  $paginationContainer.css('display', 'none');
                }else {
                  ...
                  $paginationContainer.css('display', 'block'); 
								...
								// "Fix" CSS
								console.log($curContent.outerHeight());
								...
								// Start timer again
								if( !$el.data('autoPlayStop') && settings.timeout > 0 && mouseOver !== true && $el.is(":visible")) //khamyl added isVisibl test
									autoplayInt = setTimeout( autoPlay, settings.timeout );
